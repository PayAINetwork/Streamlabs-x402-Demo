 ...
 */
export function PaywallApp({ config, onPaymentComplete, onPaymentError, bodyData }: PaywallAppProps) {
  ...
  const paymentRequirements = config
    ? selectPaymentRequirements([config.paymentRequirements].flat(), network, "exact")
    : null;
  ...
  const handlePayment = useCallback(async () => {
    if (!address || !config || !paymentRequirements) {
      return;
    }
    ...
    try {
      ...
      const validPaymentRequirements = ensureValidAmount(paymentRequirements, config.amount);
